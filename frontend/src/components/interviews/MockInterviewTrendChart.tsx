...
interface MockInterviewTrendChartProps {
  data: { date: string; percentage: number }[];
  isLoading?: boolean;
  error?: Error | null;
}

export default function MockInterviewTrendChart({
  data,
  isLoading = false,
  error = null
}: MockInterviewTrendChartProps) {
  if (isLoading) {
    return (
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 text-[var(--destructive)] shadow-sm">
        Failed to load mock interview trend.
      </div>
    );
  }
  ...
  if (data.length === 0) {
    return (
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm">
        <h3 className="mb-4 text-lg font-semibold text-[var(--foreground)]">Mock Interview Performance</h3>
